...
/* ---------- tiny helper ---------- */
function sanitize($key){ return htmlspecialchars(trim($_POST[$key] ?? '')); }
...
if ($student_type==='new'){
    ...
    if ($dup->fetchColumn()>0){
        header("Location: register_hostel.php?error=1"); exit;
    }
} else {
    $dup = $pdo->prepare("SELECT COUNT(*) FROM hostel_registrations
    ...
    if ($dup->fetchColumn()>0){
        header("Location: register_hostel_returning.php?error=1"); exit;
    }
}
...
if (hostelIsFull($pdo,$semester,$gender)){
    ...
    header("Location: " .
       ($student_type==='new' ? 'register_hostel.php' : 'register_hostel_returning.php')
       . "?full=1");
    exit;
}
...
<div style='font-family:Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:600px;margin:auto;padding:20px;background-color:#f9fafb;border-radius:8px;border:1px solid #eee;'>
  ...
  <p style='font-size:15px;color:#333;line-height:1.6; margin-top:20px;'>
    <strong>Family House Room Reservation was successful</strong> – Please check back within 24 hours for management decision and to print out <strong>2 copies</strong> of the slip if approved. These must be submitted at the accommodation verification desk upon resumption on <strong>June 26, 2025</strong>.
  </p>

  <div style='text-align:center;margin:30px 0;'>
    <a href='https://adullam.ng/dashboard/index.php' style='background-color:#6B21A8;color:white;padding:12px 24px;text-decoration:none;border-radius:6px;font-weight:bold;'>Go to Dashboard</a>
  </div>
...
$dest = ($student_type==='new') ? 'register_hostel.php' : 'register_hostel_returning.php';
