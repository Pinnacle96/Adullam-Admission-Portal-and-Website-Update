...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['focus_area'])) {
    ...
    header("Location: manage_focus_areas?program_id=$program_id");
    exit;
}
    ...
    <div class="flex flex-col lg:flex-row">
        ...
        <main class="flex-1 p-4 sm:p-6 max-w-3xl mx-auto w-full">
            <div class="bg-white p-6 rounded-xl shadow">
                ...
                <ul class="space-y-3">
                    <?php foreach ($focusAreas as $fa): ?>
                        <li
                            ...
                            <form method="POST" action="update_focus_area"
                            ...
                            <form method="POST" action="delete_focus_area" onsubmit="return confirm('Are you sure?')"
                                class="text-right">
                                <input type="hidden" name="id" value="<?= $fa['id'] ?>">
                                <input type="hidden" name="program_id" value="<?= $program_id ?>">
                ...
                <a href="manage_programs" class="inline-block mt-6 text-purple-600 hover:underline text-sm">⬅ Back
                    to Programs</a>
            </div>
        </main>
    </div>
