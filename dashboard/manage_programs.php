...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_program'])) {
    ...
    if ($name) {
        ...
        header("Location: manage_programs");
        exit;
    }
}
                    ...
                    <table class="min-w-full text-sm text-left text-gray-600">
                        ...
                        <tbody class="divide-y">
                            <?php foreach ($programs as $prog): ?>
                                <tr class="bg-white">
                                    <form method="POST" action="save_program" class="w-full">
                                        ...
                                        <td class="px-4 py-2 space-y-2">
                                            ...
                                            <?php if (strtoupper($prog['name']) === 'MA'): ?>
                                                <a href="manage_focus_areas?program_id=<?= $prog['id'] ?>"
                                                    class="text-blue-700 text-sm hover:underline">📂 Focus Areas</a><br>
                                            <?php endif; ?>
                                    </form>
                                    <form action="delete_program" method="POST"
