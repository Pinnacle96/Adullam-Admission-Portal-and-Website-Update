...
if ($userId <= 0) {
    die("Invalid applicant ID provided.");
}
...
if (!$referees) {
    die("No referees found for this applicant.");
}
...
foreach ($referees as $r) {
    ...
    $subject = '=?UTF-8?B?' . base64_encode($subject) . '?='; // UTF-8 safe
    ...
    <div style='font-family:Arial,sans-serif;padding:20px;border:1px solid #ccc;border-radius:10px;max-width:600px;margin:auto'>
        ...
        <p>If you have questions, kindly contact <a href='mailto:rcnts.adullam@gmail.com'>rcnts.adullam@gmail.com</a></p>
    ...
    if (sendMail($r['referee_email'], $r['referee_name'], $subject, $body)) {
        ...
        // Track last resend timestamp
        $update = $pdo->prepare("UPDATE application_recommendations SET last_resent_at = NOW() WHERE id = ?");
        $update->execute([$r['id']]);
    }
}
?>
